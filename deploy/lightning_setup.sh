...
#   L4+vLLM ~3s/張）。**vLLM 釘 0.10.2**：0.28+ 會讓 MinerU 輸出重複亂碼。
#   用 uv 裝 vLLM：pip 對 vLLM 依賴會 backtracking 卡到天荒地老。
# ⚠️ Turing 架構的 T4 對 vLLM 支援是邊緣的；若 T4 上 vLLM 跑不起來，退路：
#    `pip uninstall -y vllm`（MINERU_BACKEND=vlm-engine 會自動退回 transformers，慢但能動）。
pip install -U pip uv
pip install -U "mineru[core]"
pip install -U -r requirements.txt
uv pip install --python "$(which python)" "vllm==0.10.2"

...
名片常把中日韓姓名拉開字距排版。那些空白是版面效果，不是名字的一部分，
所以服務端會把它們合併：
...
**規則是：只有當每個空白分隔的 token 都是「單一 CJK 字元」時才合併。**
其餘一律只 trim 首尾、壓縮多餘的 ASCII 空白，內容不動：

| 輸入 | 輸出 | 原因 |
|---|---|---|
| `Sophie Smith` | `Sophie Smith` | 拉丁字母的空白是詞分界 |
| `Woo Jeong Hong` | `Woo Jeong Hong` | 韓文名的羅馬拼音，同樣是詞分界 |
| `Kang · Da Won` | `Kang · Da Won` | 同上 |
| `黑澤 啟一` | `黑澤 啟一` | 日文「姓 名」慣例，token 各兩字 |
| `池本　大祐` | `池本　大祐` | 全形空白 U+3000 原樣保留 |

> **不要在貴司這端再做一次 `replace(" ", "")`。** 那會把 `Sophie Smith`
> 變成 `SophieSmith`。判準是書寫系統而非語系——`Woo Jeong Hong` 與 `강 다 원`
> 是同一個語系，處置卻相反。

套用範圍：`person_name`、`english_name`、`company_name`、`job_title`。
**不套用**於 `address`（空白是結構性的）與電話／Email／網址（見上一節）。
未正規化的原樣文字永遠完整保留在 `raw_text`。

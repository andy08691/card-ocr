...
2026-09-04 對 `test/` 實測（明細見 [`../benchmarks.md`](../benchmarks.md) §C）：

| 情境 | 每張 | 100 張/天 | 200 張/天 |
|---|---:|---:|---:|
| 掃描圖（多數 <2048px 未縮） | **$0.00089（NT$0.028）** | US$2.6/月 | **US$5.2/月** |
| 手機實拍推估（縮到 2048×1536） | ~$0.00155（NT$0.050） | ~US$4.7/月 | ~US$9.3/月 |

加上 VPS（NT$130–250/月），200 張/天的**總持有成本約 NT$300–500/月**。
12 張全批實測：延遲平均 6.0s、validator issue **0**、repair **0**。

### 兩個計費上的坑（都已處理）

1. **cache write 比一般 input 貴**（$0.25 vs $0.20/1M）。OpenAI 的 prompt caching
   預設開啟，GPT-5.6 系列門檻 1,024 tokens——本管線的 prompt 都超過，所以每次請求
   都會產生 cache write。這是無法關閉的，只能設法讓它被讀回來。
2. **`prompt_cache_key` 必須是固定值**。它影響路由；每張卡一個 key 會把快取切開、
   保證永不命中。本管線用固定的 `_CACHE_KEY`，讓所有請求共用
   instructions + strict schema 前綴（實測 1,405 tokens），費率降到 $0.02/1M。
   實測**省 23.3%**。
   同理，`build_user_content` 把**圖片放在文字之前**——快取比對前綴，
   文字在前會讓同一張卡的 repair 在文字就分岔，圖片永遠進不了共用前綴。

> 快取前綴存活 30 分鐘。200 張/天 ≈ 每 7 分鐘一張，多數會命中；離峰時段會退回
> cache write，實際帳單落在上表與「零命中」（每張 $0.00116）之間。

> **成本主導項是 reasoning effort，不是圖片大小**——但已實測過，**不要為了省錢調低**。
> A/B 結果（[`../benchmarks.md`](../benchmarks.md) §D）：`none` 省 20%、快一倍，
> 但會把字形相近的漢字讀錯（「高都」→「京都」，重跑 3 次錯 2 次）並漏抓多支號碼中的一支；
> `medium` 貴 50%、慢 53% 而準確度沒有提升。`low` 是驗證過的下限。
